...
program cuentacot, nclass sortpreserve
	...
	* Contador de tiempo
	...
	quietly{
		gen int `tiempo_continuo' = floor(`time_name'/100)*12
	...
	* Contador de Cotizaciones discontinuas
		...
		quietly {
			cap drop `cuenta_tot'
			gen byte `cuenta_tot' = 1
			by `id_per': replace `cuenta_tot' = `cuenta_tot'[_n - 1] + 1 /*
				 */ *(`tiempo_continuo'[_n] != `tiempo_continuo'[_n - 1])*(`nsolic'[_n] == .) ///
				  if _n > 1 & (`nsolic'[_n - 1] == .)
	...
	* Contador de Cotizaciones continuas
		...
		quietly {		
			...
			cap drop `cot_cont'
			gen byte `cot_cont' = `cuenta_con' >= `ncot'
		}
	...
	* Contadores Con periodos
	...
	if (length("`menncont'") != 0 | length("`menndiscont'") != 0) {
		...
		quietly {
			forval i = `nper2'(-1)0 {			
				...
				by `id_per': gen int `tiempo_continuo`i'' = `tiempo_continuo'[_n - `i']
	...
	if (length("`menncont'") != 0) {
		...
		quietly {		
			...
			gen byte `cuenta_m_en_n_cont' = 1
			
			// Comparando entremedio
			tempvar cuenta_m_en_n_cont2
			gen byte `cuenta_m_en_n_cont2' = 1
	...
	if (length("`menndiscont'") != 0) {
		...
		quietly {
			...
			gen byte `cuenta_m_en_n_dis' = 1
			
			forval i = `nper2'(-1)1 {
				local j = `i' - 1
				replace `cuenta_m_en_n_dis' = `cuenta_m_en_n_dis'*(`nsolic`j'' == .) + ///
